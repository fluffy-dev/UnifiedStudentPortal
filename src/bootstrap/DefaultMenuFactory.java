...
import presentation.cli.menu.CommonMenuActions;
...
public final class DefaultMenuFactory implements MenuFactory {
    ...
    /** Shared messaging/news/request/order actions — given to every role. */
    private CommonMenuActions common(User user) {
        return new CommonMenuActions(ctx.console, user,
                ctx.messageRepository, ctx.newsRepository,
                ctx.sendMessage, ctx.commentOnNews, ctx.publishNews,
                ctx.submitRequest, ctx.createOrder);
    }
    ...
    @Override public Menu menuFor(User user) {
        if (user instanceof Admin a) {
            return new AdminMenu(ctx.console, a, ctx.userRepository, ctx.logRepository,
                    ctx.deleteUser, ctx.createStudent, ctx.generateTopResearcherNews, common(a));
        }
        if (user instanceof Manager m) {
            return new ManagerMenu(ctx.console, m, ctx.courseRepository, ctx.requestRepository,
                    ...
                    ctx.publishNews, ctx.processRequest, ctx.generateAcademicReport, common(m));
        }
        if (user instanceof Dean d) {
            return new DeanMenu(ctx.console, d, ctx.requestRepository, ctx.processRequest,
                    ctx.becomeResearcherAction, ctx.researcherMenu, common(d));
        }
        if (user instanceof Librarian l) {
            return new LibrarianMenu(ctx.console, l, ctx.bookRepository,
                    ctx.addBook, ctx.removeBook, common(l));
        }
        if (user instanceof TechSupport t) {
            return new TechSupportMenu(ctx.console, t, ctx.orderRepository,
                    ctx.acceptOrder, ctx.completeOrder, common(t));
        }
        ...
        if (user instanceof Teacher tch) {
            return new TeacherMenu(ctx.console, tch, ctx.courseRepository, ctx.recordMarks,
                    ctx.complainAboutStudent, ctx.becomeResearcherAction, ctx.researcherMenu, common(tch));
        }
        ...
        if (user instanceof Student s) {
            return new StudentMenu(ctx.console, s, ctx.courseRepository, ctx.orgRepository,
                    ...
                    ctx.becomeResearcherAction, ctx.researcherMenu, common(s));
        }
